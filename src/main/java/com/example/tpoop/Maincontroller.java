...
class MainController {
    ...
    private void ajouterCapteurDialog() {
        ...
        Label errV1 = errorLabel(""), errV2 = errorLabel(""), errV3 = errorLabel("");
        Label lv1 = dlgLbl("Val 1 :"), lv2 = dlgLbl("Val 2 :"), lv3 = dlgLbl("Val 3 :");

        cbType.setOnAction(e -> {
            switch (cbType.getValue()) {
                case ENV         -> { lv1.setText("Température :"); lv2.setText("Humidité :"); lv3.setText("Pluviométrie :"); v3.setVisible(true); errV3.setVisible(true); }
                case SOL         -> { lv1.setText("Azote :");       lv2.setText("Humidité :"); lv3.setText("pH :");           v3.setVisible(true); errV3.setVisible(true); }
                case AQUA        -> { lv1.setText("Temp. eau :");   lv2.setText("Oxygène :"); lv3.setText("pH :");            v3.setVisible(true); errV3.setVisible(true); }
                case BIOMETRIQUE -> { lv1.setText("Temp. corp. :"); lv2.setText("Activité/min :"); v3.setVisible(false); errV3.setVisible(false); }
                case GPS         -> { lv1.setText("Latitude :");    lv2.setText("Longitude :"); v3.setVisible(false); errV3.setVisible(false); }
            }
        });

        gp.addRow(0, dlgLbl("Code :"),  new VBox(2, tfCode, errCode));
        gp.addRow(1, dlgLbl("Type :"),  cbType);
        gp.addRow(2, dlgLbl("Zone :"),  cbZone);
        gp.addRow(3, lv1, new VBox(2, v1, errV1));
        gp.addRow(4, lv2, new VBox(2, v2, errV2));
        gp.addRow(5, lv3, new VBox(2, v3, errV3));
        ...
        okBtn.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
            boolean valid = true;
            if (tfCode.getText().isBlank()) { markError(tfCode, errCode, "Le code est obligatoire."); valid = false; }
            else if (!isValidName(tfCode.getText())) { markError(tfCode, errCode, "Le code doit contenir du texte (ex: CE01)."); valid = false; }
            else clearError(tfCode, errCode);
            if (!isDouble(v1.getText())) { markError(v1, errV1, "Nombre requis."); valid = false; } else clearError(v1, errV1);
            if (!isDouble(v2.getText())) { markError(v2, errV2, "Nombre requis."); valid = false; } else clearError(v2, errV2);
            if (v3.isVisible() && !isDouble(v3.getText())) { markError(v3, errV3, "Nombre requis."); valid = false; } else clearError(v3, errV3);
            if (!valid) ev.consume();
        });
        ...
        d.showAndWait().ifPresent(bt -> {
            ...
            try {
                Capteurs c = null;
                switch (cbType.getValue()) {
                    case ENV         -> c = new Cap_env(tfCode.getText().trim(), z, Status.ACTIF, dbl(v1), dbl(v2), dbl(v3));
                    case SOL         -> c = new Cap_sol(tfCode.getText().trim(), z, Status.ACTIF, dbl(v1), dbl(v2), dbl(v3));
                    case AQUA        -> c = new Cap_aqua(tfCode.getText().trim(), z, Status.ACTIF, dbl(v1), dbl(v2), dbl(v3));
                    case BIOMETRIQUE -> c = new Cap_biometrique(tfCode.getText().trim(), z, Status.ACTIF, dbl(v1), dbl(v2));
                    case GPS         -> {
                        ...
                        c = new Capteur_GPS(tfCode.getText().trim(), z, Status.ACTIF,
    ...
    private void configurerSeuilsDialog(Capteurs c) {
        ...
        if (c instanceof Cap_env ce) {
            ...
            Label e1=errorLabel(""),e2=errorLabel(""),e3=errorLabel("");
            gp.addRow(0,dlgLbl("Temp. min / max :"),    new VBox(2, new HBox(6,tTMin,new Label("/"),tTMax), e1));
            gp.addRow(1,dlgLbl("Humidité min / max :"), new VBox(2, new HBox(6,tHMin,new Label("/"),tHMax), e2));
            gp.addRow(2,dlgLbl("Pluvio. min / max :"),  new VBox(2, new HBox(6,tPMin,new Label("/"),tPMax), e3));
            ScrollPane sp = scrollPane(gp); d.getDialogPane().setContent(sp);
            Button ok = (Button)d.getDialogPane().lookupButton(ButtonType.OK);
            ok.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
                boolean v = validatePair(tTMin,tTMax,e1) & validatePair(tHMin,tHMax,e2) & validatePair(tPMin,tPMax,e3);
                if (!v) ev.consume();
            });
            ...
            Label e1=errorLabel(""),e2=errorLabel(""),e3=errorLabel("");
            gp.addRow(0,dlgLbl("Humidité min / max :"), new VBox(2, new HBox(6,tHMin,new Label("/"),tHMax), e1));
            gp.addRow(1,dlgLbl("pH min / max :"),       new VBox(2, new HBox(6,tPhMin,new Label("/"),tPhMax), e2));
            gp.addRow(2,dlgLbl("Azote min / max :"),    new VBox(2, new HBox(6,tAzMin,new Label("/"),tAzMax), e3));
            ScrollPane sp = scrollPane(gp); d.getDialogPane().setContent(sp);
            Button ok = (Button)d.getDialogPane().lookupButton(ButtonType.OK);
            ok.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
                boolean v = validatePair(tHMin,tHMax,e1) & validatePair(tPhMin,tPhMax,e2) & validatePair(tAzMin,tAzMax,e3);
                if (!v) ev.consume();
            });
            ...
            Label e1=errorLabel(""),e2=errorLabel(""),e3=errorLabel("");
            gp.addRow(0,dlgLbl("Temp. min / max :"),   new VBox(2, new HBox(6,tTMin,new Label("/"),tTMax), e1));
            gp.addRow(1,dlgLbl("Oxygène min / max :"), new VBox(2, new HBox(6,tOMin,new Label("/"),tOMax), e2));
            gp.addRow(2,dlgLbl("pH min / max :"),      new VBox(2, new HBox(6,tPhMin,new Label("/"),tPhMax), e3));
            ScrollPane sp = scrollPane(gp); d.getDialogPane().setContent(sp);
            Button ok = (Button)d.getDialogPane().lookupButton(ButtonType.OK);
            ok.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
                boolean v = validatePair(tTMin,tTMax,e1) & validatePair(tOMin,tOMax,e2) & validatePair(tPhMin,tPhMax,e3);
                if (!v) ev.consume();
            });
            ...
            Label e1=errorLabel(""),e2=errorLabel("");
            gp.addRow(0,dlgLbl("Temp. min / max :"),    new VBox(2, new HBox(6,tTMin,new Label("/"),tTMax), e1));
            gp.addRow(1,dlgLbl("Activité min / max :"), new VBox(2, new HBox(6,tAMin,new Label("/"),tAMax), e2));
            ScrollPane sp = scrollPane(gp); d.getDialogPane().setContent(sp);
            Button ok = (Button)d.getDialogPane().lookupButton(ButtonType.OK);
            ok.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
                boolean v = validatePair(tTMin,tTMax,e1) & validatePair(tAMin,tAMax,e2);
                if (!v) ev.consume();
            });
            ...
            Label e1=errorLabel(""),e2=errorLabel("");
            gp.addRow(0,dlgLbl("Longitude min / max :"), new VBox(2, new HBox(6,tLonMin,new Label("/"),tLonMax), e1));
            gp.addRow(1,dlgLbl("Latitude min / max :"),  new VBox(2, new HBox(6,tLatMin,new Label("/"),tLatMax), e2));
            ScrollPane sp = scrollPane(gp); d.getDialogPane().setContent(sp);
            Button ok = (Button)d.getDialogPane().lookupButton(ButtonType.OK);
            ok.addEventFilter(javafx.event.ActionEvent.ACTION, ev -> {
                boolean v = validatePair(tLonMin,tLonMax,e1) & validatePair(tLatMin,tLatMax,e2);
                if (!v) ev.consume();
            });
